...
def test_concurrent_ingest_200_workers(tmp_path):
    ...
    def _ingest_worker(idx_and_payload):
        ...
        try:
            return orch.process_raw_log(
                p, transport="concurrent_test", client_ip=f"10.0.0.{(idx % 250) + 1}"
            )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        futures = [
            executor.submit(_ingest_worker, (i, payload))
    ...
    assert len(errors) == 0, (
        f"Encountered {len(errors)} exceptions during concurrent ingestion: {errors[:3]}"
    )
    ...
    for e in envelopes:
        ...
        assert verify_res.is_valid is True, (
            f"Integrity failed for event {e.event_id}: {verify_res.details}"
        )
    ...
    duck_df = duck_conn.execute(
        f"SELECT COUNT(*) as cnt, COUNT(DISTINCT event_id) as distinct_cnt FROM read_parquet('{parquet_filepath}')"
    ).fetchone()
