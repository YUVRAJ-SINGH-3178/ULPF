...
class OCSFNormalizer:
    ...
    def normalize(
        ...
        time_epoch_ms, time_dt_str = self._normalize_timestamp(
        ...
        if (
            target_class_uid == OCSFClass.SECURITY_FINDING.value
            or "signature" in parsed_fields
            or "alert" in parsed_fields
        ):
            ...
            ocsf_doc = {
                ...
                or parsed_fields.get("signature")
                or "Security alert detected",
                "metadata": metadata,
                "src_endpoint": src_endpoint,
                "dst_endpoint": dst_endpoint,
                "finding_info": {
                    "title": parsed_fields.get("signature")
            ...
            action_name = parsed_fields.get("action") or (
                "allow" if disp_id == 1 else "deny"
            )
            ocsf_doc = {
                ...
                "activity_id": parsed_fields.get(
                    "activity_id", OCSFNetworkActivityId.TRAFFIC.value
                ),
                ...
                "time_dt": time_dt_str,
                "message": parsed_fields.get("message")
                or f"{vendor} {product} network event",
                "metadata": metadata,
                "src_endpoint": src_endpoint,
                "dst_endpoint": dst_endpoint,
                "connection_info": conn_info,
                "traffic": traffic,
                "app_name": parsed_fields.get("app_name")
                or parsed_fields.get("service"),
                "unmapped": unmapped,
            }
    ...
    def _build_connection_info(self, parsed: dict[str, Any]) -> dict[str, Any]:
        ...
        dir_id = (
            ...
            else (2 if direction.lower() == "outbound" else 3)
        )
