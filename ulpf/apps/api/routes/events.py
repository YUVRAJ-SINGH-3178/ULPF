...
def get_orchestrator() -> PipelineOrchestrator:
    if not _orchestrator:
        raise HTTPException(
            status_code=500, detail="Pipeline orchestrator not initialized"
        )
...
def ingest_single_event(
    ...
    user: dict[str, Any] = Depends(
        require_roles([UserRole.ADMIN, UserRole.OPERATOR, UserRole.ANALYST])
    ),
):
    """
...
def ingest_batch_events(
    ...
    orch = get_orchestrator()
    envelopes = orch.process_batch(
        req.logs,
        transport=req.transport or "batch",
        client_ip=req.client_ip or "127.0.0.1",
    )
    return {
        "status": "success",
...
def search_events(
    query: str | None = Query(
        None, description="Full-text search query across raw and OCSF documents"
    ),
    vendor: str | None = Query(
        None, description="Filter by vendor name (e.g. Cisco, Palo Alto, Fortinet)"
    ),
    product: str | None = Query(
        None, description="Filter by product name (e.g. ASA, PAN-OS, FortiOS)"
    ),
    detected_format: str | None = Query(None, description="Filter by detected format"),
    severity_id: int | None = Query(
        None, description="Filter by OCSF severity_id (1-6)"
    ),
    disposition: str | None = Query(
        None, description="Filter by disposition (Allowed, Blocked)"
    ),
...
def get_event_details(event_id: str, user: dict[str, Any] = Depends(get_current_user)):
    """
    ...
    if not event_data:
        raise HTTPException(
            status_code=404, detail=f"Event {event_id} not found in search index"
        )
    return event_data


@router.get("/{event_id}/raw", response_model=dict[str, Any])
def get_raw_event(event_id: str, user: dict[str, Any] = Depends(get_current_user)):
    """
    ...
    if not res:
        raise HTTPException(
            status_code=404, detail=f"Raw event {event_id} not found in raw storage"
        )
...
def tamper_test_helper(
    event_id: str, user: dict[str, Any] = Depends(require_roles([UserRole.ADMIN]))
):
    """
