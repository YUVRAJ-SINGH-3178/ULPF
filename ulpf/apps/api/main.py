    ...
    if settings.ULPF_ENABLE_SYSLOG_LISTENERS:
        try:
            udp_listener = UDPSyslogListener(
                ...
                callback=lambda env: orchestrator.process_raw_log(
                    env.raw.raw_payload, transport="udp", client_ip=env.source.client_ip
                ),
            )
            udp_listener.start()
        except Exception:
        ...
        try:
            tcp_listener = TCPSyslogListener(
                ...
                envelope_factory=orchestrator.envelope_factory,
                callback=lambda env: orchestrator.process_raw_log(
                    env.raw.raw_payload, transport="tcp", client_ip=env.source.client_ip
                ),
            )
            tcp_listener.start()
        except Exception:
...
app.mount(
    "/static", StaticFiles(directory=str(dashboard_static_dir.resolve())), name="static"
)
